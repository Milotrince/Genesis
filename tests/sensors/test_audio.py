...
def test_modal_analysis_material_ordering():
    ...
    verts, elems = eu.mesh_to_elements(trimesh.creation.box(extents=(0.2, 0.03, 0.03)))
...
# This test needs a higher carrier rate than the other audio tests: `from_mesh` drops modes at or above
# 0.45 * Nyquist, and at K=80 (16 kHz) only 2 of the aluminium bar's 6 modes survive, leaving one of them
# dominant at every strike point. Timbre can only shift with strike location when several modes are audible.
TAP_K = 160
TAP_STRIKE_X = -0.10
...
def _tap_spectrum(show_viewer, strike_x, use_surface):
    ...
    verts, elems = eu.mesh_to_elements(trimesh.creation.box(extents=bar_size))
...
def test_position_dependent_timbre(show_viewer):
    ...
    d_surface = divergence(
        _tap_spectrum(show_viewer, strike_x=TAP_STRIKE_X, use_surface=True),
    ...
    d_flat = divergence(
        _tap_spectrum(show_viewer, strike_x=TAP_STRIKE_X, use_surface=False),
