...
def test_apply_external_wrench(xml_path, show_viewer, tol):
    ...
    # Match the solver's center-of-mass frame for the aligned free link
    duck_init_link_pos = duck.base_link.get_pos(relative=False)
...
def test_dofs_force_balance_advanced(contact_resolution, show_viewer, tol):
    ...
    hand = scene.add_entity(
        gs.morphs.URDF(
            ...
            euler=(-90, 0, 0),
            fixed=True,
            # The grasp state below was calibrated on the collision-derived masses, and the residual the assertions
            # bound is that of the solve at the default iteration count on this state.
