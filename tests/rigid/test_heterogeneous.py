...
def test_variant_switch_notifies_geometry_subscribers(n_envs, show_viewer):
    ...
    # A fixed link carries no DOF, so a configuration-space setter can never announce its geometry swap; the switch
    # must announce it directly, or a static subscriber like the raycaster's collision BVH never rebuilds.
    subscriber = Subscriber(
        to=frozenset({StateChange.GEOMETRY}),
        links_filter=np.array([link.idx for link in het.links]),
    )
    het.solver.subscribe(subscriber)
    het.set_entity_variant(1)
    assert StateChange.GEOMETRY in subscriber.pending
